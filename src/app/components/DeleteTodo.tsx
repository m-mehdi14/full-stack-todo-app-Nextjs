...
const DeleteTodo = () => {
  console.log('DeleteTodo mounted');
    ...
    <section className="text-gray-600 body-font">
      <div className="container px-5 py-24 mx-auto">
        <div className="flex flex-col text-center w-full mb-20">
          <h1 className="text-3xl font-medium title-font mb-4 text-gray-900">MY TODOS</h1>
        ...
        <div className="flex md:flex-row md:flex-wrap -m-4 sm:flex-col sm:flex-wrap flex-col ">
          {todos.map((todo) => (
            <div className="p-4 lg:w-1/4 md:w-1/2 flex-row " key={todo.id}>
              <div className="h-full flex flex-col items-center text-center">
                <img
                  alt="team"
                  className="flex-shrink-0 rounded-lg w-full h-56 object-cover object-center mb-4"
                  src={`https://picsum.photos/600/900?a=${todo.title}`}
                />
                <div className="w-full">
                  <h2 className="title-font font-medium text-lg text-gray-900">{todo.title}</h2>
                  <p className="mb-4">todo-id: {todo.id}</p>
                  <p className="mb-4 text-center">{todo.description}</p>
                  <span className="inline-flex">
                
                      <a className="text-gray-500 cursor-pointer" onClick={() => handleDelete(todo.id)} >
                   ...
                   {/* edit svg */}
                    ...
                    <Link className="ml-2 text-gray-500 cursor-pointer" href={`/edit/${todo.id}`}>
